...
<div class="page-header">
  <div class="page-header-left"><h2>Edit Department</h2><p><?= htmlspecialchars($department['name']) ?></p></div>
  <a href="<?= url('departments') ?>" class="btn btn-ghost">← Back</a>
</div>
<div class="card" style="max-width:600px">
  <div class="card-body">
    <form method="POST" action="<?= url('departments/update/'.$department['id']) ?>" class="form-grid">
      <?= csrf_field() ?>
      <div class="form-group">
        <label>Name <span class="req">*</span></label>
        <input type="text" name="name" value="<?= htmlspecialchars($department['name']) ?>"
      ...
      <div class="form-group full">
        ...
        <select name="parent_id" class="form-control">
          ...
          <?php foreach($departments as $d): if($d['id']==$department['id']) continue; ?>
          <option value="<?= $d['id'] ?>" <?= ($department['parent_id']??'')==$d['id']?'selected':'' ?>><?= htmlspecialchars($d['name']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group full">
      ...
      <div class="form-actions full">
        <button type="submit" class="btn btn-primary btn-lg">Update Department</button>
